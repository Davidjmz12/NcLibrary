...
```{r}
head(nc_open("data/example.nc"),4)
```
### Data Formatting

The next step involves formatting the file.   This is accomplished using the `read_nc_file()` which returns a dataframe with the information of the file. In this instance, we will employ the basic functionality of the function. For further details, please consult  `help("read_nc_file")`.
